...
%%%%% homogeneous turbulence using eigenmodes constructed by GSI
...
%%
eig_Us = zeros(1,Ny*Nz,Nt_mod);
eig_Uc = zeros(1,Ny*Nz,Nt_mod);
eig_Vs = zeros(1,Ny*Nz,Nt_mod);
eig_Vc = zeros(1,Ny*Nz,Nt_mod);
eig_Ws = zeros(1,Ny*Nz,Nt_mod);
eig_Wc = zeros(1,Ny*Nz,Nt_mod);

for iix = 1:Nt_mod

    fid=fopen([dir_ModeData,'uuS_GSI_iX-',num2str(iix),'.bin']);
    aaa = reshape(fread(fid,'float32'),[Ny,Nz])./sqrt(2);
    eig_Us(1,:,iix) = aaa(:);
    fclose(fid);
...
ftn = permute(ft(1:Nt_mod),[3 2 1]);

sx = sin(ftn.*tn');
cx = cos(ftn.*tn');

uu_GSI = reshape(sum(( sx.*eig_Us + cx.*eig_Uc ) , 3),[Nt_CFD,Ny,Nz])+Umean;
vv_GSI = reshape(sum(( sx.*eig_Vs + cx.*eig_Vc ) , 3),[Nt_CFD,Ny,Nz]);
...
savefig('TimeSeries_divergence_Cubic.fig')

%%% Instantaneous velocity field
[xx1,yy1] = meshgrid(tn*Umean,y);
[xx2,zz2] = meshgrid(tn*Umean,z);
...
savefig('TimeSeries_velocity_Cubic.fig')
